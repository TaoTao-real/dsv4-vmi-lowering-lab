...
func.func @prefill_idx_qr_proj_aiv(%arg0: !pto.ptr<i8, gm>, %arg1: !pto.ptr<i8, gm>, %arg2: !pto.ptr<f32, gm>, %arg3: !pto.ptr<f32, gm>, %arg4: !pto.ptr<f32, gm>, %arg5: i32, %arg6: i32) attributes {pto.kernel_kind = #pto.kernel_kind<vector>} {
  ...
  scf.for %arg7 = %c0 to %c128 step %c16 {
    ...
    %16 = pto.alloc_tile addr = %c132096_i64 valid_row = %c16 valid_col = %c256 : !pto.tile_buf<vec, 16x256xf32, valid=?x?>
    pto.trowexpandmul ins(%13, %14 : !pto.tile_buf<vec, 16x256xf32, valid=?x?>, !pto.tile_buf<vec, 16x1xf32, valid=?x?, blayout=col_major>) outs(%16 : !pto.tile_buf<vec, 16x256xf32, valid=?x?>) {candidates = [{id = 0 : i64, loop_depth = 2 : i64, name = "template_trowexpandmul", postupdate = 0 : i64, tags = ["row_expand", "binary"], tail = 0 : i64}], pto.tilelib.impl = "ptodsl", pto.tilelib.selected_candidate = {id = 0 : i64, loop_depth = 2 : i64, name = "template_trowexpandmul", postupdate = 0 : i64, tags = ["row_expand", "binary"], tail = 0 : i64}, pto.vmi.fusion.boundary = "local", pto.vmi.fusion.boundary_reason = "non_vmi_local_boundary_fallback"}
    %17 = pto.fusion_region {
      %19 = pto.alloc_tile addr = %c132096_i64 valid_row = %c16 valid_col = %c256 : !pto.tile_buf<vec, 16x256xf32, valid=?x?>
      pto.tcolexpandmul ins(%16, %11 : !pto.tile_buf<vec, 16x256xf32, valid=?x?>, !pto.tile_buf<vec, 1x256xf32, valid=?x?>) outs(%19 : !pto.tile_buf<vec, 16x256xf32, valid=?x?>) {candidates = [{id = 0 : i64, loop_depth = 2 : i64, name = "template_tcolexpandmul", postupdate = 0 : i64, tags = ["column_expand", "binary"], tail = 0 : i64}, {id = 1000 : i64, loop_depth = 1 : i64, name = "vmi_tcolexpandmul", postupdate = 0 : i64, tags = ["vmi", "fusion_eligible", "single_logical_row_loop"], tail = 0 : i64}], pto.tilelib.impl = "vmi", pto.tilelib.selected_candidate = {id = 1000 : i64, loop_depth = 1 : i64, name = "vmi_tcolexpandmul", postupdate = 0 : i64, tags = ["vmi", "fusion_eligible", "single_logical_row_loop"], tail = 0 : i64}}
      pto.yield(%19) : (!pto.tile_buf<vec, 16x256xf32, valid=?x?>) -> ()
    } {pto.fusion.group_id = 0 : i64} : !pto.tile_buf<vec, 16x256xf32, valid=?x?>
    %18 = pto.partition_view %3, offsets = [%c0, %c0, %c0, %arg7, %8], sizes = [%c1, %c1, %c1, %c16, %c256] : !pto.tensor_view<1x1x1x?x?xf32>
    pto.tstore ins(%17 : !pto.tile_buf<vec, 16x256xf32, valid=?x?>) outs(%18 : !pto.partition_tensor_view<1x1x1x16x256xf32>) {candidates = [{id = 0 : i64, loop_depth = 3 : i64, name = "template_tstore_nd", postupdate = 0 : i64, tags = ["store", "ub", "gm", "nd"], tail = 0 : i64}], layout = #pto.layout<nd>, pto.tilelib.impl = "ptodsl", pto.tilelib.selected_candidate = {id = 0 : i64, loop_depth = 3 : i64, name = "template_tstore_nd", postupdate = 0 : i64, tags = ["store", "ub", "gm", "nd"], tail = 0 : i64}, pto.vmi.fusion.boundary = "hard", pto.vmi.fusion.boundary_reason = "non_vmi_hard_boundary_fallback"}
  }
  return
}
