    ...
    scf.if %34 {
      scf.for %arg15 = %c0 to %c2 step %c1 {
        ...
        %45 = pto.fusion_region {
          ...
          pto.tmov ins(%44 : !pto.tile_buf<vec, 32x128xbf16, valid=?x?>) outs(%69 : !pto.tile_buf<vec, 32x128xbf16, valid=?x?, blayout=col_major, slayout=row_major>) {candidates = [{id = 8 : i64, loop_depth = 1 : i64, name = "template_tmov_nd2nz", postupdate = 1 : i64, tags = ["move", "ub", "ub", "nd2nz", "nz"], tail = 0 : i64}, {id = 1000 : i64, loop_depth = 1 : i64, name = "vmi_tmov", postupdate = 0 : i64, tags = ["vmi", "fusion_eligible", "single_logical_row_loop", "supports_partial_valid_shape"], tail = 0 : i64}], pto.tilelib.impl = "vmi", pto.tilelib.selected_candidate = {id = 1000 : i64, loop_depth = 1 : i64, name = "vmi_tmov", postupdate = 0 : i64, tags = ["vmi", "fusion_eligible", "single_logical_row_loop", "supports_partial_valid_shape"], tail = 0 : i64}}
          pto.yield(%69) : (!pto.tile_buf<vec, 32x128xbf16, valid=?x?, blayout=col_major, slayout=row_major>) -> ()
        } {pto.fusion.group_id = 7 : i64} : !pto.tile_buf<vec, 32x128xbf16, valid=?x?, blayout=col_major, slayout=row_major>
        pto.tpush(%45, %14 : !pto.tile_buf<vec, 32x128xbf16, valid=?x?, blayout=col_major, slayout=row_major>, !pto.pipe) {split = 0}
        %46 = arith.muli %arg15, %c2 : index
        %47 = arith.muli %46, %c80 : index
        %48 = arith.addi %26, %47 : index
        %49 = arith.muli %25, %c16 : index
        %50 = arith.addi %48, %49 : index
        %51 = pto.alloc_tile addr = %c82432_i64 valid_row = %c16 valid_col = %c1 : !pto.tile_buf<vec, 16x1xf32, valid=?x?, blayout=col_major>
        %52 = pto.subview %39[%c0, %c0] sizes [16, 1] : !pto.tile_buf<vec, 32x1xf32, valid=?x?, blayout=col_major> -> !pto.tile_buf<vec, 16x1xf32, blayout=col_major>
        %53 = pto.partition_view %1, offsets = [%c0, %c0, %c0, %50, %c0], sizes = [%c1, %c1, %c1, %c16, %c1] : !pto.tensor_view<1x1x1x?x?xf32>
        pto.tstore ins(%52 : !pto.tile_buf<vec, 16x1xf32, blayout=col_major>) outs(%53 : !pto.partition_tensor_view<1x1x1x16x1xf32>) {candidates = [{id = 1 : i64, loop_depth = 3 : i64, name = "template_tstore_dn", postupdate = 0 : i64, tags = ["store", "ub", "gm", "dn"], tail = 0 : i64}], layout = #pto.layout<dn>, pto.tilelib.impl = "ptodsl", pto.tilelib.selected_candidate = {id = 1 : i64, loop_depth = 3 : i64, name = "template_tstore_dn", postupdate = 0 : i64, tags = ["store", "ub", "gm", "dn"], tail = 0 : i64}, pto.vmi.fusion.boundary = "hard", pto.vmi.fusion.boundary_reason = "non_vmi_hard_boundary_fallback"}
        %54 = pto.alloc_tile addr = %c82560_i64 valid_row = %c16 valid_col = %c1 : !pto.tile_buf<vec, 16x1xf32, valid=?x?, blayout=col_major>
        %55 = pto.subview %43[%c0, %c0] sizes [16, 1] : !pto.tile_buf<vec, 32x1xf32, valid=?x?, blayout=col_major> -> !pto.tile_buf<vec, 16x1xf32, blayout=col_major>
        %56 = pto.partition_view %0, offsets = [%c0, %c0, %c0, %50, %c0], sizes = [%c1, %c1, %c1, %c16, %c1] : !pto.tensor_view<1x1x1x?x?xf32>
        pto.tstore ins(%55 : !pto.tile_buf<vec, 16x1xf32, blayout=col_major>) outs(%56 : !pto.partition_tensor_view<1x1x1x16x1xf32>) {candidates = [{id = 1 : i64, loop_depth = 3 : i64, name = "template_tstore_dn", postupdate = 0 : i64, tags = ["store", "ub", "gm", "dn"], tail = 0 : i64}], layout = #pto.layout<dn>, pto.tilelib.impl = "ptodsl", pto.tilelib.selected_candidate = {id = 1 : i64, loop_depth = 3 : i64, name = "template_tstore_dn", postupdate = 0 : i64, tags = ["store", "ub", "gm", "dn"], tail = 0 : i64}, pto.vmi.fusion.boundary = "hard", pto.vmi.fusion.boundary_reason = "non_vmi_hard_boundary_fallback"}
        %57 = arith.muli %arg15, %c2 : index
        %58 = arith.addi %57, %c1 : index
        %59 = arith.muli %58, %c80 : index
        %60 = arith.addi %26, %59 : index
        %61 = arith.muli %25, %c16 : index
        %62 = arith.addi %60, %61 : index
        %63 = pto.alloc_tile addr = %c82432_i64 valid_row = %c16 valid_col = %c1 : !pto.tile_buf<vec, 16x1xf32, valid=?x?, blayout=col_major>
        %64 = pto.subview %39[%c16, %c0] sizes [16, 1] : !pto.tile_buf<vec, 32x1xf32, valid=?x?, blayout=col_major> -> !pto.tile_buf<vec, 16x1xf32, blayout=col_major>
        %65 = pto.partition_view %1, offsets = [%c0, %c0, %c0, %62, %c0], sizes = [%c1, %c1, %c1, %c16, %c1] : !pto.tensor_view<1x1x1x?x?xf32>
        pto.tstore ins(%64 : !pto.tile_buf<vec, 16x1xf32, blayout=col_major>) outs(%65 : !pto.partition_tensor_view<1x1x1x16x1xf32>) {candidates = [{id = 1 : i64, loop_depth = 3 : i64, name = "template_tstore_dn", postupdate = 0 : i64, tags = ["store", "ub", "gm", "dn"], tail = 0 : i64}], layout = #pto.layout<dn>, pto.tilelib.impl = "ptodsl", pto.tilelib.selected_candidate = {id = 1 : i64, loop_depth = 3 : i64, name = "template_tstore_dn", postupdate = 0 : i64, tags = ["store", "ub", "gm", "dn"], tail = 0 : i64}, pto.vmi.fusion.boundary = "hard", pto.vmi.fusion.boundary_reason = "non_vmi_hard_boundary_fallback"}
        %66 = pto.alloc_tile addr = %c82560_i64 valid_row = %c16 valid_col = %c1 : !pto.tile_buf<vec, 16x1xf32, valid=?x?, blayout=col_major>
        %67 = pto.subview %43[%c16, %c0] sizes [16, 1] : !pto.tile_buf<vec, 32x1xf32, valid=?x?, blayout=col_major> -> !pto.tile_buf<vec, 16x1xf32, blayout=col_major>
        %68 = pto.partition_view %0, offsets = [%c0, %c0, %c0, %62, %c0], sizes = [%c1, %c1, %c1, %c16, %c1] : !pto.tensor_view<1x1x1x?x?xf32>
        pto.tstore ins(%67 : !pto.tile_buf<vec, 16x1xf32, blayout=col_major>) outs(%68 : !pto.partition_tensor_view<1x1x1x16x1xf32>) {candidates = [{id = 1 : i64, loop_depth = 3 : i64, name = "template_tstore_dn", postupdate = 0 : i64, tags = ["store", "ub", "gm", "dn"], tail = 0 : i64}], layout = #pto.layout<dn>, pto.tilelib.impl = "ptodsl", pto.tilelib.selected_candidate = {id = 1 : i64, loop_depth = 3 : i64, name = "template_tstore_dn", postupdate = 0 : i64, tags = ["store", "ub", "gm", "dn"], tail = 0 : i64}, pto.vmi.fusion.boundary = "hard", pto.vmi.fusion.boundary_reason = "non_vmi_hard_boundary_fallback"}
      }
    } else {
