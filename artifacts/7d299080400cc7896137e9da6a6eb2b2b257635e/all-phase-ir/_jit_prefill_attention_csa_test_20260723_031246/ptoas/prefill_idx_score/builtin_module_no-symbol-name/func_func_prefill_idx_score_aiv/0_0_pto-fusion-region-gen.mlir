  ...
  scf.for %arg9 = %c0 to %c8 step %c1 {
    ...
    scf.if %16 {
      ...
      scf.for %arg10 = %c0 to %c128 step %c1 {
        ...
        %33 = pto.alloc_tile addr = %c73728_i64 valid_row = %c1 valid_col = %c64 : !pto.tile_buf<vec, 1x64xf32, valid=?x?>
        %34 = pto.partition_view %7, offsets = [%c0, %c0, %c0, %arg10, %c0], sizes = [%c1, %c1, %c1, %c1, %c64] : !pto.tensor_view<1x1x1x?x?xf32>
        pto.tload ins(%34 : !pto.partition_tensor_view<1x1x1x1x64xf32>) outs(%33 : !pto.tile_buf<vec, 1x64xf32, valid=?x?>) {candidates = [{id = 0 : i64, loop_depth = 3 : i64, name = "template_tload_nd2nd", postupdate = 0 : i64, tags = ["load", "gm", "ub", "nd"], tail = 0 : i64}], layout = #pto.layout<nd>, pto.tilelib.impl = "ptodsl", pto.tilelib.selected_candidate = {id = 0 : i64, loop_depth = 3 : i64, name = "template_tload_nd2nd", postupdate = 0 : i64, tags = ["load", "gm", "ub", "nd"], tail = 0 : i64}, pto.vmi.fusion.boundary = "hard", pto.vmi.fusion.boundary_reason = "non_vmi_hard_boundary_fallback"}
        pto.fusion_region {
          %47 = pto.alloc_tile addr = %c65536_i64 valid_row = %c32 valid_col = %c64 : !pto.tile_buf<vec, 32x64xf32, valid=?x?>
          pto.tcolexpandmul ins(%32, %33 : !pto.tile_buf<vec, 32x64xf32, valid=?x?>, !pto.tile_buf<vec, 1x64xf32, valid=?x?>) outs(%47 : !pto.tile_buf<vec, 32x64xf32, valid=?x?>) {candidates = [{id = 0 : i64, loop_depth = 2 : i64, name = "template_tcolexpandmul", postupdate = 0 : i64, tags = ["column_expand", "binary"], tail = 0 : i64}, {id = 1000 : i64, loop_depth = 1 : i64, name = "vmi_tcolexpandmul", postupdate = 0 : i64, tags = ["vmi", "fusion_eligible", "single_logical_row_loop"], tail = 0 : i64}], pto.tilelib.impl = "vmi", pto.tilelib.selected_candidate = {id = 1000 : i64, loop_depth = 1 : i64, name = "vmi_tcolexpandmul", postupdate = 0 : i64, tags = ["vmi", "fusion_eligible", "single_logical_row_loop"], tail = 0 : i64}}
          %48 = pto.alloc_tile addr = %c73984_i64 valid_row = %c32 valid_col = %c128 : !pto.tile_buf<vec, 32x128xf32, valid=?x?>
          %49 = pto.alloc_tile addr = %c90496_i64 valid_row = %c32 valid_col = %c1 : !pto.tile_buf<vec, 32x1xf32, valid=?x?, blayout=col_major>
          pto.trowsum ins(%47, %48 : !pto.tile_buf<vec, 32x64xf32, valid=?x?>, !pto.tile_buf<vec, 32x128xf32, valid=?x?>) outs(%49 : !pto.tile_buf<vec, 32x1xf32, valid=?x?, blayout=col_major>) {candidates = [{id = 0 : i64, loop_depth = 2 : i64, name = "template_trowsum", postupdate = 0 : i64, tags = ["reduction", "row", "sum"], tail = 0 : i64}, {id = 1000 : i64, loop_depth = 1 : i64, name = "vmi_trowsum", postupdate = 0 : i64, tags = ["vmi", "fusion_eligible", "single_logical_row_loop"], tail = 0 : i64}], pto.tilelib.impl = "vmi", pto.tilelib.selected_candidate = {id = 1000 : i64, loop_depth = 1 : i64, name = "vmi_trowsum", postupdate = 0 : i64, tags = ["vmi", "fusion_eligible", "single_logical_row_loop"], tail = 0 : i64}}
          pto.yield() : () -> ()
        } {pto.fusion.group_id = 1 : i64} :
        %35 = pto.alloc_tile addr = %c90496_i64 valid_row = %c1 valid_col = %c32 : !pto.tile_buf<vec, 1x32xf32, valid=?x?>
        %36 = pto.load_scalar %arg0[%arg10] : !pto.ptr<i32, gm> -> i32
        %37 = arith.index_cast %36 : i32 to index
        %38 = arith.addi %37, %c1 : index
        %39 = arith.divsi %38, %c4 : index
        %40 = arith.minsi %39, %c256 : index
        %41 = arith.cmpi slt, %15, %40 : index
        %42 = scf.if %41 -> (index) {
          %47 = arith.subi %40, %15 : index
          %48 = arith.minsi %47, %c32 : index
          scf.yield %48 : index
        } else {
          scf.yield %c0 : index
        }
        pto.set_validshape %35, %c1, %42 : !pto.tile_buf<vec, 1x32xf32, valid=?x?>
        %43 = pto.alloc_tile addr = %c73984_i64 valid_row = %c1 valid_col = %c32 : !pto.tile_buf<vec, 1x32xf32, valid=?x?, pad=3>
        pto.tfillpad ins(%35 : !pto.tile_buf<vec, 1x32xf32, valid=?x?>) outs(%43 : !pto.tile_buf<vec, 1x32xf32, valid=?x?, pad=3>) {candidates = [{id = 0 : i64, loop_depth = 2 : i64, name = "template_tfillpad", postupdate = 0 : i64, tags = ["fillpad"], tail = 0 : i64}], pto.tilelib.impl = "ptodsl", pto.tilelib.selected_candidate = {id = 0 : i64, loop_depth = 2 : i64, name = "template_tfillpad", postupdate = 0 : i64, tags = ["fillpad"], tail = 0 : i64}, pto.vmi.fusion.boundary = "hard", pto.vmi.fusion.boundary_reason = "non_vmi_hard_boundary_fallback"}
        %44 = pto.alloc_tile addr = %c65536_i64 valid_row = %c1 valid_col = %c32 : !pto.tile_buf<vec, 1x32xf32, valid=?x?>
        pto.texpands ins(%cst_0 : f32) outs(%44 : !pto.tile_buf<vec, 1x32xf32, valid=?x?>) {candidates = [{id = 0 : i64, loop_depth = 2 : i64, name = "template_texpands", postupdate = 0 : i64, tags = ["elementwise", "scalar", "fill"], tail = 0 : i64}], pto.tilelib.impl = "ptodsl", pto.tilelib.selected_candidate = {id = 0 : i64, loop_depth = 2 : i64, name = "template_texpands", postupdate = 0 : i64, tags = ["elementwise", "scalar", "fill"], tail = 0 : i64}, pto.vmi.fusion.boundary = "local", pto.vmi.fusion.boundary_reason = "non_vmi_local_boundary_fallback"}
        %45 = pto.fusion_region {
          %47 = pto.alloc_tile addr = %c73984_i64 valid_row = %c1 valid_col = %c32 : !pto.tile_buf<vec, 1x32xf32, valid=?x?, pad=3>
          pto.tmax ins(%43, %44 : !pto.tile_buf<vec, 1x32xf32, valid=?x?, pad=3>, !pto.tile_buf<vec, 1x32xf32, valid=?x?>) outs(%47 : !pto.tile_buf<vec, 1x32xf32, valid=?x?, pad=3>) {candidates = [{id = 0 : i64, loop_depth = 2 : i64, name = "template_tmax", postupdate = 0 : i64, tags = ["elementwise", "binary"], tail = 0 : i64}, {id = 1000 : i64, loop_depth = 1 : i64, name = "vmi_tmax", postupdate = 0 : i64, tags = ["vmi", "fusion_eligible", "single_logical_row_loop"], tail = 0 : i64}], pto.tilelib.impl = "vmi", pto.tilelib.selected_candidate = {id = 1000 : i64, loop_depth = 1 : i64, name = "vmi_tmax", postupdate = 0 : i64, tags = ["vmi", "fusion_eligible", "single_logical_row_loop"], tail = 0 : i64}}
          pto.yield(%47) : (!pto.tile_buf<vec, 1x32xf32, valid=?x?, pad=3>) -> ()
        } {pto.fusion.group_id = 2 : i64} : !pto.tile_buf<vec, 1x32xf32, valid=?x?, pad=3>
        %46 = pto.partition_view %1, offsets = [%c0, %c0, %c0, %arg10, %15], sizes = [%c1, %c1, %c1, %c1, %c32] : !pto.tensor_view<1x1x1x?x?xf32>
        pto.tstore ins(%45 : !pto.tile_buf<vec, 1x32xf32, valid=?x?, pad=3>) outs(%46 : !pto.partition_tensor_view<1x1x1x1x32xf32>) {candidates = [{id = 0 : i64, loop_depth = 3 : i64, name = "template_tstore_nd", postupdate = 0 : i64, tags = ["store", "ub", "gm", "nd"], tail = 0 : i64}], layout = #pto.layout<nd>, pto.tilelib.impl = "ptodsl", pto.tilelib.selected_candidate = {id = 0 : i64, loop_depth = 3 : i64, name = "template_tstore_nd", postupdate = 0 : i64, tags = ["store", "ub", "gm", "nd"], tail = 0 : i64}, pto.vmi.fusion.boundary = "hard", pto.vmi.fusion.boundary_reason = "non_vmi_hard_boundary_fallback"}
      }
    } else {
    }
  }
